...
//! `$XDG_DATA_HOME/neural-forge/captures` (see `neural_forge_layer::dump`). A manual trigger for
...
//!
//! Respects `$NEURAL_FORGE_SHM`/`$NEURAL_FORGE_UID` the same way every other tool in this
...
fn main() {
    ...
    println!("trigger_capture: capture_request set -- check $XDG_DATA_HOME/neural-forge/captures on the layer's next present");
}
